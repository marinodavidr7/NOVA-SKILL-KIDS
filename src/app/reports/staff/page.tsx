...
export default async function ReportPage({ searchParams }: { searchParams: { month?: string } }) {
  ...
  const reportData = await getStaffReport();
  ...
  return (
    <div className="min-h-screen bg-slate-100 print:bg-white pb-20">
          ...
          <div className="flex justify-between items-start border-b-2 border-slate-800 pb-6 mb-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">Reporte de Personal</p>
              </div>
            </div>
          ...
          <table className="w-full text-left text-sm border-collapse mb-12">
            <thead>
              <tr className="border-b border-slate-300 bg-slate-50">
                ...
                <th className="py-3 px-4 font-semibold text-slate-800">Nombre</th>
                <th className="py-3 px-4 font-semibold text-slate-800">Puesto</th>
                <th className="py-3 px-4 font-semibold text-slate-800">Teléfono</th>
                <th className="py-3 px-4 font-semibold text-slate-800">Ingreso</th>
              </tr>
            </thead>
            <tbody>
      ...
      <td className="py-3 px-4 text-slate-600">{new Date(item.hireDate).toLocaleDateString('es-DO')}</td>
