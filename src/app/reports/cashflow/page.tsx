...
export default async function ReportPage({ searchParams }: { searchParams: { month?: string } }) {
  ...
  const reportData = await getCashflowReport(currentMonth);
          ...
          <div className="flex justify-between items-start border-b-2 border-slate-800 pb-6 mb-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">Reporte de Flujo de Caja (Ledger)</p>
              </div>
            </div>
          ...
          <table className="w-full text-left text-sm border-collapse mb-12">
            <thead>
              <tr className="border-b border-slate-300 bg-slate-50">
                ...
                <th className="py-3 px-4 font-semibold text-slate-800">Fecha</th>
                <th className="py-3 px-4 font-semibold text-slate-800">Tipo</th>
                <th className="py-3 px-4 font-semibold text-slate-800">Descripción</th>
            ...
            <tbody>
              {reportData.length === 0 && true ? (
                ...
                reportData.map((item, index) => (
                  <tr key={`${item.source || ''}-${item.id || index}`} className="border-b border-slate-200 last:border-b-0">
