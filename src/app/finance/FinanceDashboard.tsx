  ...
  return (
    <div className="space-y-8 animate-fade-in pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="border border-slate-100 dark:border-slate-800/60 shadow-sm relative overflow-hidden bg-white dark:bg-slate-900/50 dark:backdrop-blur-xl">
          <div className="absolute top-0 right-0 p-4 opacity-10 dark:opacity-5"><DollarSign className="w-24 h-24" /></div>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-500 dark:text-slate-400">Ingresos del Mes</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-black text-emerald-600 dark:text-emerald-400">${summary.totalIncome.toLocaleString()}</div>
        ...
        <Card className="border border-slate-100 dark:border-slate-800/60 shadow-sm relative overflow-hidden bg-white dark:bg-slate-900/50 dark:backdrop-blur-xl">
          ...
          <CardContent>
            <div className="text-3xl font-black text-rose-600 dark:text-rose-400">${summary.totalExpenses.toLocaleString()}</div>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-1">
        ...
        <Card className="border border-slate-100 dark:border-slate-800/60 shadow-sm relative overflow-hidden bg-white dark:bg-slate-900/50 dark:backdrop-blur-xl">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-500 dark:text-slate-400">Balance del Periodo</CardTitle>
          </CardHeader>
          <CardContent>
            <div className={`text-3xl font-black ${summary.netProfit >= 0 ? 'text-indigo-600 dark:text-indigo-400' : 'text-rose-600 dark:text-rose-400'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Liquidity Widget */}
          <Card className="border border-slate-800 shadow-sm bg-gradient-to-r from-slate-900 to-slate-800 text-white overflow-hidden relative">
          ...
          {/* Recent Transactions */}
          <Card className="border border-slate-100 dark:border-slate-800/60 shadow-sm bg-white dark:bg-slate-900/50 dark:backdrop-blur-xl">
            <CardHeader className="border-b border-slate-100 dark:border-slate-800/60 flex flex-row items-center justify-between pb-4">
              <CardTitle className="text-base font-bold text-slate-800 dark:text-slate-100">Transacciones Recientes</CardTitle>
              <Link href="/finance/accounts" className="text-sm text-indigo-600 dark:text-indigo-400 font-medium hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors">Ver todas</Link>
            </CardHeader>
                    ...
                    <div key={tx.id} className="p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      <div className="flex items-center gap-4">
                        <div className={`h-10 w-10 rounded-full flex items-center justify-center shrink-0 ${
                          tx.type === 'in' ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400' :
                          tx.type === 'out' ? 'bg-rose-100 dark:bg-rose-500/20 text-rose-600 dark:text-rose-400' :
                        ...
                        <div>
                          ...
                          <div className="flex gap-2 items-center text-xs text-slate-500 dark:text-slate-400">
                            ...
                            {tx.category && (
                              <>
                                <span>•</span>
                                <span className="bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded text-slate-600 dark:text-slate-300">{tx.category}</span>
                              </>
                            )}
                          </div>
                        </div>
                      ...
                      <div className={`font-bold shrink-0 ${
                        ...
                        tx.type === 'out' ? 'text-rose-600 dark:text-rose-400' :
        ...
        <div className="space-y-6">
          <Card className="border border-slate-100 dark:border-slate-800/60 shadow-sm bg-white dark:bg-slate-900/50 dark:backdrop-blur-xl">
            <CardHeader className="border-b border-slate-100 dark:border-slate-800/60 pb-4">
              <CardTitle className="text-base font-bold text-slate-800 dark:text-slate-100">Gastos por Categoría</CardTitle>
            </CardHeader>
            <CardContent className="pt-6">
              {expensesByCategory.length === 0 ? (
                <div className="text-center text-slate-500 dark:text-slate-400 py-8">No hay gastos registrados este mes.</div>
              ) : (
                <div className="space-y-6">
                  {expensesByCategory.map((exp: any, index: number) => (
                    <div key={index}>
                      <div className="flex justify-between text-sm mb-2">
                        ...
                        <span className="font-bold text-slate-900 dark:text-white">${exp.total.toLocaleString()}</span>
                      </div>
